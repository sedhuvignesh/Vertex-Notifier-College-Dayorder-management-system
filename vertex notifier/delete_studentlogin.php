...
if (isset($_GET['del']) && $_GET['del'] !== '') {
    ...
    $sql = "DELETE FROM student_login WHERE username = ?";

    $stmt = mysqli_prepare($connection, $sql);

    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "s", $delete); // Assuming 'username' is a VARCHAR type, use "s" for string

        if (mysqli_stmt_execute($stmt)) {
            mysqli_stmt_close($stmt);
            mysqli_close($connection);
            header("Location: studentlogin_control.php"); // Redirect using header
            exit();
        } else {
